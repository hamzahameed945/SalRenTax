...
If the variable is missing, the build falls back to `https://payrentax.me` so canonical URLs, the sitemap and `robots.txt` never point at a placeholder domain.

Production uses `PUBLIC_SITE_URL=https://payrentax.me`. Set the same value as a build variable in the Cloudflare dashboard (Workers Builds → Settings → Variables and secrets), otherwise canonical URLs, the sitemap and `robots.txt` fall back to the placeholder.

## Keeping the workers.dev address out of search results

`public/_headers` adds `X-Robots-Tag: noindex, nofollow` to responses served from `*.workers.dev`, so the temporary Cloudflare address is not indexed while `payrentax.me` stays indexable. Do not add a `Disallow` rule for it in `robots.txt`: crawlers must be able to fetch a page to see the noindex header.
