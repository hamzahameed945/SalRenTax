...
describe('buildWebSiteJsonLd', () => {
  it('builds a valid WebSite entry', () => {
    const result = buildWebSiteJsonLd('SalRenTax', 'https://salrentax.com');
    expect(result['@type']).toBe('WebSite');
    expect(result.url).toBe('https://salrentax.com');
...
describe('buildWebPageJsonLd', () => {
  it('builds a valid WebPage entry with no undefined fields', () => {
    const result = buildWebPageJsonLd({
      ...
      url: 'https://salrentax.com/en-us/salary/paycheck-calculator/',
      inLanguage: 'en',
    });
...
describe('buildBreadcrumbJsonLd', () => {
  it('numbers positions starting at 1 in order', () => {
    const result = buildBreadcrumbJsonLd([
      { label: 'Home', url: 'https://salrentax.com/en-us/' },
      { label: 'Salary', url: 'https://salrentax.com/en-us/salary/' },
    ]);
