...
describe('calculator engine coverage', () => {
  ...
  it('covers the German gross-to-social-net engine', () => {
    const result = bruttoNettoEngine.calculate(
      { grossMonthly: 5000, childless: true },
      undefined as never,
      2026,
    );
    expect(result.totalSocialInsuranceEmployee).toBeGreaterThan(0);
    expect(result.netAfterSocialInsuranceMonthly).toBeLessThan(5000);
    expect(result.incomeTaxIncluded).toBe(false);
  });
  ...
  it('covers the Spain salary engine', () => {
    ...
    expect(result.seguridadSocialAnnual).toBeGreaterThan(0);
  ...
  it('covers the Mexico salary engine', () => {
    ...
    expect(result.isrMonthly).toBeGreaterThanOrEqual(0);
    expect(result.netMonthly).toBeLessThan(15000);
    expect(result.partialImssOnly).toBe(true);
  });
