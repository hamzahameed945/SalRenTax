...
export default function PaycheckCalculator({ stateCode }: { stateCode?: string }) {
  ...
  }, [grossPay, payFrequency, filingStatus, preTaxDeductions, stateCode]);
        ...
        {result ? (
          <div class="space-y-3">
            ...
            <dl class="mt-4 space-y-2 text-sm text-slate-700">
              ...
              {stateCode && (result.stateIncomeTaxPerPeriod ?? 0) > 0 && (
                <div class="flex justify-between border-b border-slate-200 pb-1">
                  <dt>State income tax ({stateCode})</dt>
                  <dd>-{formatCurrency(result.stateIncomeTaxPerPeriod ?? 0, config)}</dd>
                </div>
              )}
