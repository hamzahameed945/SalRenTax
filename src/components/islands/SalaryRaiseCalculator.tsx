...
export default function SalaryRaiseCalculator({ t, locale }: { t: ErrorTranslations; locale: string }) {
  ...
  const result = useMemo(() => {
    const input = {
      ...
      newSalary:
        raiseType === 'flat' ? parseFloat(currentSalary) + parseFloat(raiseValue) : undefined,
    };
  ...
  return (
    <div class="grid gap-8 md:grid-cols-2">
      <div class="space-y-6">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Raise Type</label>
          <select
            class="w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
            value={raiseType}
            onChange={(e) =>
              setRaiseType((e.target as HTMLSelectElement).value as 'percentage' | 'flat')
            }
          >
        ...
        <div>
          ...
          <input
            ...
            placeholder={raiseType === 'percentage' ? 'e.g. 5' : 'e.g. 5000'}
          />
      ...
      <div class="rounded-xl bg-slate-50 p-6">
        <h3 class="text-lg font-semibold text-slate-900 mb-4">Raise Impact</h3>
        {result.data ? (
          <div class="space-y-4">
            <div>
              <p class="text-sm text-slate-500">New Annual Salary</p>
              <p class="text-3xl font-bold text-accent">{formatCurrency(result.data.newSalary)}</p>
            </div>
            <div class="pt-4 border-t border-slate-200">
              <p class="text-sm text-slate-500 mb-1">Total Increase</p>
              <p class="font-medium text-green-600">
                +{formatCurrency(result.data.increaseAmount)} (
                {formatPercent(result.data.increasePercentage)})
              </p>
            </div>
            <div class="pt-4 border-t border-slate-200">
              <p class="text-sm text-slate-500 mb-1">Monthly Increase</p>
              <p class="font-medium text-green-600">
                +{formatCurrency(result.data.monthlyDifference)}
              </p>
            </div>
            <div class="pt-4 border-t border-slate-200">
              <p class="text-sm text-slate-500 mb-1">Bi-weekly Increase</p>
              <p class="font-medium text-green-600">
                +{formatCurrency(result.data.biweeklyDifference)}
              </p>
            </div>
            <div class="pt-4 border-t border-slate-200">
              <p class="text-sm text-slate-500 mb-1">Hourly Increase</p>
              <p class="font-medium text-green-600">
                +{formatCurrency(result.data.hourlyDifference || 0)}
              </p>
            </div>
          </div>
        ) : (
          <p class="text-slate-500 text-sm">
            Enter your current salary and expected raise to see the impact.
          </p>
        )}
      </div>
    </div>
